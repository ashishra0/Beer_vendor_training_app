...
def beer_vendor
    ...
    orders_json_hash.each do |name|
        each_user_order.store(name['user'], name['drink']+ " " + name['size'])
    end
    # puts each_user_order
    ...
    price = [
    prices_json_hash.collect(&beer_small_rates) - [nil],
    prices_json_hash.collect(&beer_medium_rates) - [nil],
    prices_json_hash.collect(&beer_large_rates) - [nil],
    prices_json_hash.collect(&beer_huge_rates) - [nil],
    prices_json_hash.collect(&beer_mega_rates) - [nil],
    prices_json_hash.collect(&beer_ultra_rates) - [nil]
    ]
